...
print("Building token sequences with SpaCy …")
import spacy
...
lemma_dict = {}
word_index = 1
docs = nlp.pipe(train_texts + test_texts, n_threads=2)
    ...
    for token in doc:
        if token.pos_ != "PUNCT":
            if token.text not in word_dict:
                ...
                word_index += 1
            seq.append(word_dict[token.text])
...
test_seqs = sequences[num_train:]

# Pad
train_seqs = pad_sequences(train_seqs, maxlen=PARAMS["max_length"], padding="post")
test_seqs = pad_sequences(test_seqs, maxlen=PARAMS["max_length"], padding="post")
...
def train_and_save(emb_matrices, model_name_suffix):
    with mlflow.start_run(run_name=model_name_suffix):
        ...
        local_path = os.path.join(
            os.getenv("LOCAL_MODEL_PATH", "/tmp"), f"{model_name_suffix}_{timestamp}.h5"
        )
